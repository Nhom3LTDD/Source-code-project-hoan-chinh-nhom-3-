...
export default class CauHoi3 extends React.Component {
  constructor(props){
    super(props);
  ...
  componentDidMount(){
    return fetch('http://192.168.3.166:82/TracNghiem/CauHoi3.php')
  ...
  render() {
    ...
    return (
      ...
      <Text style={{ marginBottom: 20, textAlign: 'center', backgroundColor: 'green', fontSize: 20, color: "white" }}>CÂU HỎI TRẮC NGHIỆM</Text>
       <Text style={{ marginBottom: 20 }}>CÂU HỎI 3:</Text>  
       <Text style={{ marginBottom: 20 }}>Hãy chọn câu trả lời chính xác:</Text>  
        ...
        <TouchableOpacity style={styles.cauhoi} onPress={() => this.resultFalse()}>
        <Text style = {styles.text}>A: 6</Text>
                </TouchableOpacity>
        <TouchableOpacity style={styles.cauhoi2} onPress={() => this.resultTrue()}>
        <Text style = {styles.text}>B: 4</Text>
                </TouchableOpacity>
        </View>
        ...
        <Button
          title="Tiếp"
          color="grey"
          onPress={() => this.props.navigation.navigate('CauHoi4')}
        /> 
        <Button
          title="Đáp Án"
          color="red"
